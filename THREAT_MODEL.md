...
| Inventory tampering | Authorization on every API, complete-schema validation, per-document Sanity revision gating (`ifRevisionId`), atomic lock-document uniqueness checks, status-transition validation |
...
1. **Inventory uniqueness is an application-level convention, not a database constraint.** Sanity has no native cross-document unique-field guarantee, so stock number/VIN/slug uniqueness is enforced by creating a `vehicleLock` document in the same all-or-nothing transaction as the vehicle write; a collision fails the whole transaction. This depends entirely on every writer going through this application's code path — a direct Sanity dashboard edit or a stolen API token can create a document that bypasses the lock entirely, since nothing at the Sanity level enforces it. Per-document revision gating (`ifRevisionId`) similarly depends on every writer using it; a tool that ignores `_rev` can still overwrite a newer edit. Transaction atomicity and `ifRevisionId` semantics were verified against Sanity's current documentation and SDK type definitions, not against a live dataset under real concurrent load — that remains a deployment verification item.
