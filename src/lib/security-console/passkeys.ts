...
export function getWebAuthnConfig(headers: Headers): WebAuthnConfig {
  const configuredRpID = (process.env.EMBEDDED_SECURITY_WEBAUTHN_RP_ID || process.env.SECURITY_WEBAUTHN_RP_ID)?.trim();
  const configuredOrigin = (process.env.EMBEDDED_SECURITY_WEBAUTHN_ORIGIN || process.env.SECURITY_WEBAUTHN_ORIGIN)?.trim();
