...
export type PasswordResult = { ok: true; epoch: number } | { ok: false; reason: "invalid" | "unavailable" | "already_configured" };

export async function isPasswordConfigured() {
  const redis = getSecurityRedis();
  if (!redis) return null;
  return Boolean(await redis.get(CREDENTIAL_KEY));
}
...
export async function initializePassword(bootstrapToken: string, password: string): Promise<PasswordResult> {
  ...
  const result = await redis.eval(
    "if redis.call('EXISTS', KEYS[1]) == 1 then return 0 end; redis.call('SET', KEYS[1], ARGV[1]); redis.call('SET', KEYS[2], '1'); return 1",
    [CREDENTIAL_KEY, EPOCH_KEY], [hash],
  );
...
export async function verifyPassword(password: string): Promise<PasswordResult> {
  ...
  if (!hash || !Number.isSafeInteger(epoch) || epoch < 1) return { ok: false, reason: "invalid" };
  try { return await argon2.verify(hash, password) ? { ok: true, epoch } : { ok: false, reason: "invalid" }; }
