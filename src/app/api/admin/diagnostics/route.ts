...
import { readBugReports } from "@/lib/bug-reports";
...
export async function GET() {
  return withDiagnostics("ADMIN_DIAGNOSTICS_READ", async () => {
    ...
    const [events, reports] = await Promise.all([readDiagnostics(), readBugReports()]);
    ...
    return Response.json({ events, reports, catalog }, { headers: privateResponseHeaders() });
  });
}
