  ...
  async function loginWithPasskey(passwordJustVerified = false) {
    ...
    try {
      ...
      if (!verifyResponse.ok || verifyResult.error || !verifyResult.authenticated) throw new Error(verifyResult.error || "Passkey could not be verified.");
        window.location.assign("/Security_Console?authenticated=1");
    } catch (error) {
  ...
  return (
    <>
      <form className="panel" onSubmit={loginWithPassword}>
        ...
        <input id="security-console-password" type={showPassword ? "text" : "password"} autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} required />
