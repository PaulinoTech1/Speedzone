...
type PasskeySummary = { id: string; deviceType: string; backedUp: boolean; name?: string };
...
export default function AdminPasskey({ authenticated, onAuthenticated }: { authenticated: boolean; onAuthenticated?: () => void }) {
  ...
  async function run(action: "register" | "authenticate") {
    const passkeyName = action === "register" ? window.prompt("Name this passkey (for example, Office MacBook):", "")?.trim() : "";
    if (action === "register" && !passkeyName) {
      setMessage("Enter a name for this passkey to continue.");
      return;
    }
    ...
    try {
      ...
      const result = await fetch("/api/admin/passkey", { method: "POST", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, response, ...(action === "register" ? { name: passkeyName } : {}) }) });
      const resultPayload = await result.json(); if (!result.ok) throw new Error(resultPayload.error || "Passkey verification failed.");
      setMessage(action === "register" ? "Passkey added. You can use it next time." : "Signed in with passkey."); onAuthenticated?.(); if (action === "register") await loadPasskeys();
  ...
  async function removePasskey(id: string) {
    if (!window.confirm("Delete this passkey? Make sure you can still sign in with the another passkey. Recovery resets access if needed.")) return;
  ...
  return <section className="admin-card passkey-card"><p className="eyebrow">FIDO2 security</p><h2>Passkey sign-in</h2><p>Enroll your first passkey after password setup. Additional passkeys require a current passkey session. Accepted: platform authenticators (Touch ID, Face ID, Windows Hello, Android screen lock) and roaming FIDO2 security keys (USB, NFC, Bluetooth).</p><p className="muted">Passwords, SMS codes, OTPs, and magic links are not passkeys.</p><button className="button button-secondary" type="button" onClick={() => run("register")} disabled={busy}>{busy ? "Waiting for authenticator…" : "Add passkey"}</button><div className="passkey-list"><h3>Registered passkeys</h3>{passkeys.length === 0 ? <p className="muted">No passkeys registered yet.</p> : passkeys.map((passkey, index) => <div className="passkey-row" key={passkey.id}><div><strong>{passkey.name || `Passkey ${index + 1}`}</strong><p className="muted">{passkey.deviceType === "singleDevice" ? "Single-device authenticator" : "Multi-device authenticator"}{passkey.backedUp ? " · Backed up" : ""}</p></div><button className="button button-danger" type="button" onClick={() => removePasskey(passkey.id)} disabled={busy}>Delete</button></div>)}</div>{message && <p className="form-message" role="status">{message}</p>}</section>;
}
