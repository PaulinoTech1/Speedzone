...
export async function POST(request: Request) {
  ...
  if (!result.ok) {
    await recordConsoleAudit(request,"console.login",result.reason==="unavailable"?"unavailable":"denied",result.reason);
    ...
    return NextResponse.json({ error: "Invalid credentials" }, { status: 401, headers: privateHeaders });
  }
