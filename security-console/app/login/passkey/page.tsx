...
export default async function PasskeyLoginPage(){
  ...
  const [count, recoveryAvailable] = await Promise.all([establishedPasskeyCount(), passkeyRecoveryAvailable()]);
  ...
  </section>{count !== null && recoveryAvailable && <PasskeyRecovery/>}</main>
}
